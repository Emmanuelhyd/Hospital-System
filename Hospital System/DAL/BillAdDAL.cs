                ...
                foreach (DataRow row in dt.Rows)
                    billAds.Add(
                        new BillAd
                        {
                            ...
                            DoctorFee = Convert.ToInt64(row["DoctorFee"]),
                            TreatmentDuration = Convert.ToInt32(row["TreatmentDuration"]),
                            TreatmentCharges = Convert.ToInt64(row["TreatmentCharges"]),
                            MedicineCharges = Convert.ToInt64(row["MedicineCharges"]),
                            RoomFee = Convert.ToInt64(row["RoomFee"]),
                            Others = row["Others"].ToString(),
                            OthersCost = Convert.ToInt64(row["OthersCost"]),
                            TotalBill = Convert.ToInt64(row["TotalBill"]),
                            GST = Convert.ToInt64(row["GST"]),
                            TotalAmount=Convert.ToInt64(row["TotalAmount"]),
                            InsuranceClaimed = Convert.ToInt64(row["InsuranceClaimed"]),
                            PaidBill = Convert.ToInt64(row["PaidBill"]),
        ...
        public BillAd BillEdit(int PatientId)
        {
            ...
            SqlCommand cmd = new SqlCommand("Select * from Bill where PatientId='" + PatientId + "'", con);
            {
                ...
                if (reader.Read())
                {
                    ...
                    billAd.DoctorFee = Convert.ToInt64(reader["DoctorFee"]);
                    billAd.TreatmentDuration = Convert.ToInt32(reader["TreatmentDuration"]);
                    billAd.TreatmentCharges = Convert.ToInt64(reader["TreatmentCharges"]);
                    billAd.MedicineCharges = Convert.ToInt64(reader["MedicineCharges"]);
                    billAd.RoomFee = Convert.ToInt64(reader["RoomFee"]);
                    billAd.Others = reader["Others"].ToString();
                    billAd.OthersCost = Convert.ToInt64(reader["OthersCost"]);
                    billAd.TotalBill = Convert.ToInt64(reader["TotalBill"]);
                    billAd.GST = Convert.ToInt64(reader["GST"]);
                    billAd.TotalAmount = Convert.ToInt64(reader["TotalAmount"]);
                    billAd.InsuranceClaimed = Convert.ToInt64(reader["InsuranceClaimed"]);
                    billAd.PaidBill = Convert.ToInt64(reader["PaidBill"]);
